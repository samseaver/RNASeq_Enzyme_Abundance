...
class Species:
    ...
    def modelJSON_file_path(self, modelJSON_file_path):
        ...
        if self.metModel == None:
            raise ValueError(bcolors.FAIL+f"  Couldn't load metabolic model from {modelJSON_file_path}."+bcolors.ENDC)
...
def jsonModelPath2Var(csp, verbose=True):
    json_dict = dict()
    if verbose: print(bcolors.PROMPT+"-"*10+" Updating JSON file paths "+"-"*10)
    ...
    for fileName in fileNames:
        for spc in csp.project_species:
            ...
            and ('070224'.lower() in fileName.lower()):
...
def rnaSeqFile2Var(csp, verbose=True):
    ...
    if verbose: print(bcolors.PROMPT+"-"*10+" Updating RNASeq file paths "+"-"*10)
...
def readRNASeq(spc, csp, verbose=True):
    print(bcolors.PROG+f" Computing Protein weights for {spc.name} ..."+bcolors.ENDC)
    ...
    percent = -1
    dist_dict = dict()
    groups = relab_df.groupby(csp.group_columns)
    for name, group in groups:
        dist_dict[name] = np.abs(group[csp.value_column])\
            .describe([csp.outlier_cap_percentile/100])[str(csp.outlier_cap_percentile)+'%']
        # dist_dict[name] = dist
    print(dist_dict)
    # print(abc)

    for name, cap_value in dist_dict.items():
        relab_df.loc[((relab_df["tissue"] == name[0]) & 
                    (relab_df["treatment"] == name[1]) & 
                    (relab_df["time_stamp"] == name[2]) & 
                    (relab_df[csp.value_column] > cap_value))
        , csp.value_column] = cap_value

    #     # relab_df[csp.value_column] = relab_df[csp.value_column]
    

    bin_width= 10
    # here you can choose your rounding method, I've chosen math.ceil
    nbins = math.ceil((relab_df[csp.value_column].max() - relab_df[csp.value_column].min()) / bin_width)
    fig = px.histogram(relab_df, x=csp.value_column, nbins=nbins, title='relab')
    fig.show()
    ...
    pwg = ProteinWeightGenerator(spc.name, csp.project, spc.RNASeq_file_path,
                                ...
                                csp.group_columns, cap_percent=dist_dict)
...
def readTMMdata(spc, csp, verbose=True):
    value_col = csp.value_column #'value'
    # Read RANSeq data from file
    print(spc.RNASeq_file_path)
    ...
    bin_width= 10
    # fig = px.histogram(tmm_df, x=value_col)
    wt, ht = 800, 600
    fig = px.violin(tmm_df,
                    x='time_stamp',
                    y='value',
                    color="treatment",
                    title=spc.name+" before cap",
                    height=ht, width=wt
                )
    # fig.show()

    # here you can choose your rounding method, I've chosen math.ceil
    nbins = math.ceil((tmm_df[value_col].max() - tmm_df[value_col].min()) / bin_width)
    fig = px.histogram(tmm_df, x=value_col, nbins=nbins, title='obj')
    # fig.show()
    ...
    fig = px.violin(tmm_df,
                    ...
                    y='value',
                    color="treatment",
                    title=spc.name+" model only",
                    height=ht, width=wt
                )
    # fig.show()
    
    nbins = math.ceil((tmm_df[value_col].max() - tmm_df[value_col].min()) / bin_width)
    fig = px.histogram(tmm_df, x=value_col, nbins=nbins)
    # fig.show()

    # percent = tmm_df[value_col].\
    ...
    percent = -1
    dist_dict = dict()
    groups = tmm_df.groupby(csp.group_columns)
    # print(groups)
    ...
    for name, cap_value in dist_dict.items():
        tmm_df.loc[((tmm_df["tissue"] == name[0]) & 
                    (tmm_df["treatment"] == name[1]) & 
                    (tmm_df["time_stamp"] == name[2]) & 
                    (tmm_df[csp.value_column] > cap_value))
        , csp.value_column] = cap_value
    ...
    print(tmm_df['value'].max())
    print(tmm_df.groupby(csp.group_columns)[value_col].describe())

    print(bcolors.PROG +f"\tThere are {len(spc.metModel.modelfeatures_dict)} genes in the model")
    print(bcolors.PROG +f"\t          {len(tmm_df[csp.rnaSeq_id_col].unique())} genes in the RNAseq")
    # print(abc)
...
def run_pipeline(parameters_class, project:str='QPSI', project_species:list=[], verbose=True):
    ...
    if verbose:
        for spc, json_file in json_dict.items():
            print(bcolors.PROG+"        "+spc+" <-- "+json_file+bcolors.ENDC)
    rnaseq_dict = rnaSeqFile2Var(csp)
    if verbose:
        for spc, rnaseq_file in rnaseq_dict.items():
            print(bcolors.PROG+"        "+spc+" <-- "+rnaseq_file+bcolors.ENDC)
    # set model species
    ...
    # ## Metal binding ==============================================================================
    # for metal in csp.metals:
    #     print(bcolors.PROMPT+"-"*10+" Reading RDGB prediction {}".format(metal)+"-"*10+bcolors.ENDC)
    #     rdgbFilePath2Var(csp, metal)
    #     if verbose:
    #         for spc in csp.species_list:
    #             print(bcolors.PROG+"        "+spc.name +" <-- "+spc.pred_file_path+bcolors.ENDC)


    #     # Processing RDGB predictions:
    #     rdgbHelperObj = RDGBHelper({spc.name:spc.pred_file_path for spc in csp.species_list})
    #     # read RDGB predictions for all species
    #     rdgbHelperObj.runHelper()
    #     # Get all species id_pfam_dictionaries
    #     AllIdPfam_dict = rdgbHelperObj.getIdPfamDict('all')

    #     # Update each species with the binding properties for its list of model features
    #     for spc in csp.species_list:
    #         print(bcolors.PROMPT+"-"*10+" PROCESSING "+bcolors.BOLD+bcolors.UNDERLINE
    #             +spc.name+bcolors.ENDC+bcolors.PROMPT+"-"*10+bcolors.ENDC)
    #         print("    Computing intersection between prediction and metabolic model.")
    #         spc.updateMetalBinding(AllIdPfam_dict[spc.name], metal)
    #         # write list of metal binding genes to a file.
    #         with open(csp.results_folder+spc.name+"_"+metal+"_allPredictedGenes.csv", "w") as outfile:
    #             outfile.write("\n".join(list(AllIdPfam_dict[spc.name].keys())))

    # ## Consolidate models =========================================================================
    # # Create one model with all project species information for comparison
    # metal = csp.metals[0]
    # roles_dict_copy = consilidated_model(csp, metal)
    # json_output = json.dumps(list(roles_dict_copy.values()), indent=4)
    # output_json_file = csp.results_folder+metal+'_consolidated_output_prediction_model.json'
    # with open(output_json_file, 'w') as outfile:
    #     outfile.write(json_output)
    # print(bcolors.RESULT+"Results printed to "+
    #             bcolors.BOLD+ "{}"
    #             .format(output_json_file)+bcolors.ENDC)

    ## Reactions cores ============================================================================
    for spc in csp.species_list:

        print(bcolors.PROMPT+"\n"+"-"*10+"    Computing reaction scores "+"-"*10+bcolors.ENDC)
        ...
        print(bcolors.PROG +"    Computing reactions scores using TMM data"+bcolors.ENDC)
        ...
        print(bcolors.SUBRESULT +"        ** {} Model Reactions.".format(spc.name)+bcolors.ENDC)
        relab_df, pwg = readRNASeq(spc, csp)

        ## Start relative abundamce based reaction score computation ----------------------------
        print(bcolors.PROG +"    Computing reactions scores"+bcolors.ENDC)
        ...
        # rxn_scores.to_csv(f"temp_{spc.name}_scores.csv")
        ...
        print(bcolors.PROMPT+"Writing reaction scores, binding and details to:")
        print(csp.results_folder+spc.name+"_rxn_scores_"+csp.msr+".csv"+bcolors.ENDC)

        # rxn_scores['value'] = rxn_scores['value']/6.02214076e+23
        print(rxn_scores.describe(percentiles=[.25, .5, .75, .8, .9, .95]))
        # rxn_scores['value'] = rxn_scores['value'] * 6.02214076e+23
        # print(abc)
        rxn_scores.to_csv(csp.results_folder+spc.name+"_relab_rxn_scores_"+csp.msr+".csv", index=False)
        rxn_scores[(rxn_scores['treatment'].isin(['Control', 'FeLim'])) & (rxn_scores['tissue'] == 'Leaf') ].to_csv(csp.results_folder+spc.name+"_relab_rxn_scores_14d_FeLim_"+csp.msr+".csv", index=False)
...
def plotMethodDiffs(csp, spc, tmm_df, tmm_scores, relative_scores, pwg, verbose=False):
    ...
    # tmm_old_scores = tmm_old_scores[tmm_old_scores['value'] >= 95]
    # Create figure
    # fig = go.Figure()
    #
    # # Add scatter traces
    # fig.add_trace(go.Scatter(x=tmm_old_scores['rxn_ID'],
    #                         y=tmm_old_scores['rxn_dist_quantile_max'],
    #                         mode="markers"))
    # fig.add_trace(go.Scatter(x=tmm_old_scores['rxn_ID'],
    #                         y=tmm_old_scores['rxn_dist_quantile_sum'],
    #                         mode="markers"))
    #
    # # Add shapes
    # fig.add_shape(type="circle",
    #     xref="x", yref="y",
    #     x0=0, y0=min(tmm_old_scores['rxn_dist_quantile_max']),
    #     x1=100, y1=max(tmm_old_scores['rxn_dist_quantile_max']),
    #     opacity=0.2,
    #     fillcolor="blue",
    #     line_color="blue",
    # )
    ...
    # fig.add_shape(type="circle",
    #     xref="x", yref="y",
    #     x0=0, y0=min(tmm_old_scores['rxn_dist_quantile_sum']),
    #     x1=100, y1=max(tmm_old_scores['rxn_dist_quantile_sum']),
    #     opacity=0.2,
    #     fillcolor="orange",
    #     line_color="orange",
    # )
...
## ## -------------------- CONSOLIDATED DICT ------------------------
# Create one consolidated model with all species and
#  their binding properties.
#  Model is saved to file for comparison, but is not used in
#  the pipeline.
def consilidated_model(csp, metal):
    roles_dict_copy = roles_dict.copy()
    all_rxns = set(csp.species_list[0].metModel.modelreactions_dict.keys())

    for rxn_id in all_rxns:
        rxn_roles_dict = dict()
        for s_ind in range(len(csp.species_list)):
            spc = csp.species_list[s_ind]
            rxn = spc.metModel.modelreactions_dict[rxn_id]
            rxn_role_dict = rxn.role_properties(metal)
            for role, properties in rxn_role_dict.items():
                item = None
                if role in rxn_roles_dict:
                    item = rxn_roles_dict[role]
                else:
                    item = {"id": rxn_id,
                                "predicted_genes": dict(),
                                "features": dict(),
                                "PFAM_domains": list(),
                                "compartment": list(),
                                "bind": list()
                                }
                item["predicted_genes"][spc.name] = list(properties["predicted_genes"])
                item["features"][spc.name] = list(properties["features"])
                item["PFAM_domains"] = list(set().union(properties['PFAM_domains'],\
                                                    item["PFAM_domains"]))
                item["compartment"] = list(set().union([rxn.compartment],\
                                                    item["compartment"]))

                item["bind"].append(properties["bind"])
                if (s_ind  == len(csp.species_list)-1) and (len(set(item["bind"]))>1):
                    print(bcolors.WARNING+"Binding properties vary across species")
                    print(bcolors.PROG+f"    {rxn_id} for role: {role}")
                    print(item)
                    # print(abc)

                rxn_roles_dict[role] = item

                if s_ind  == len(csp.species_list)-1:
                    if role in roles_dict_copy:
                        try:
                            roles_dict_copy[role]["reactions"].append(item)
                        except KeyError:
                            roles_dict_copy[role]["reactions"] = [item]
                            # print(roles_dict_copy[role])
    return roles_dict_copy
## ## ---------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    # project = Parameters_secMeta_TSU
    project = Parameters_QPSI
    # project = Parameters_secMeta
    run_pipeline(project)


    # ###### -------- Find the list of reactions with missing proteins compared to A. thaliana
    # athalianaModelJson = "/Users/sea/QPSI_project/QPSI_Modeling/data/metabolic_models/plastidial_models/athaliana_plastidial_model.json"
    # # athalianaModelJson = "/Users/sea/QPSI_project/QPSI_Modeling/data/metabolic_models/fullmodels_media/athaliana_tair10_model.json"
    # otherSpcModelJson = "/Users/sea/QPSI_project/QPSI_Modeling/data/metabolic_models/plastidial_models/sbicolor_plastidial_model.json"
    # sorghum = checkProteinLists(athalianaModelJson, otherSpcModelJson)
    # print("*"*60)
    # otherSpcModelJson = "/Users/sea/QPSI_project/QPSI_Modeling/data/metabolic_models/plastidial_models/sbicolor_plastidial_model.json"
    # otherSpcModelJson = "/Users/sea/QPSI_project/QPSI_Modeling/data/metabolic_models/plastidial_models/ptrich_plastidial_model.json"
    # otherSpcModelJson = "/Users/sea/QPSI_project/QPSI_Modeling/data/metabolic_models/fullmodels_media/athaliana_tair10_model.json"
    # # poplar = checkProteinLists(athalianaModelJson, otherSpcModelJson)
    # rxn2genes(otherSpcModelJson)
    #
    #
    # print(f"Sorghum {len(sorghum)} - Poplar {len(poplar)}")
    # print(f"Intersection {sorghum.intersection(poplar)}")
    # print("Sorghum")
    # print('\n'.join([id for id in sorghum]))
    # print("Poplar")
    # print('\n'.join([id for id in poplar]))


    # ###### -------- Generate data for reaction score bar figure
    # rna = pa.read_csv("/Users/sea/QPSI_project/QPSI_Modeling/data/RNASeq_data/tmm/Poplar_raw_genes_tmm_mean_std.csv")
    #
    # genes = ["Potri.005G079200", "Potri.006G241600", "Potri.006G260200", "Potri.007G088426", "Potri.018G022200", "Potri.013G113900", "Potri.007G137950", "Potri.017G013800", "Potri.017G013900", "Potri.017G014000", "Potri.017G014100", "Potri.017G014200"]
    #
    # # rxn00781
    # genes = ["Potri.002G007100", "Potri.002G220566", "Potri.005G254100", "Potri.008G083900", "Potri.010G172400", "Potri.014G140500"]
    #
    # # rxn00533_d0
    # genes = ["Potri.005G169100", "Potri.006G095800", "Potri.016G110500", "Potri.013G162600", "Potri.019G054400", "Potri.006G078200", "Potri.018G146300"]
    #
    # rna = rna[rna['Gene_ID'].isin(genes)]
    #
    # for name, group in rna[(rna['time_stamp'] == '7d') & (rna['tissue']=='Root') & (rna['treatment'].isin(['Control', 'FeLim']))].groupby('treatment'):
    #     # & (rna['treatment'].isin(['Control', 'FeLim']))
    #     print(group[['Gene_ID', 'treatment', 'value']])
    #     print(group.shape)
